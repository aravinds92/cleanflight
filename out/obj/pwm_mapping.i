...
# 35 "src/main/drivers/pwm_mapping.c" 2
...
# 36 "src/main/drivers/pwm_mapping.c" 2
# 44 "src/main/drivers/pwm_mapping.c"
...
# 83 "src/main/drivers/pwm_mapping.c"
enum {
    MAP_TO_PPM_INPUT = 1,
    MAP_TO_PWM_INPUT,
    MAP_TO_MOTOR_OUTPUT,
...
pwmIOConfiguration_t *pwmInit(drv_pwm_config_t *init)
{
    ...
    for (i = 0; i < 11 && setup[i] != 0xFFFF; i++) {
        uint8_t timerIndex = setup[i] & 0x00FF;
        uint8_t type = (setup[i] & 0xFF00) >> 8;


        const timerHardware_t *timerHardwarePtr = 
# 711 "src/main/drivers/pwm_mapping.c" 3 4
                                                 ((void *)0)
# 711 "src/main/drivers/pwm_mapping.c"
                                                     ;




        timerHardwarePtr->gpio->pin = 2;
        timerHardwarePtr->gpio->mode = PWM;
        timerHardwarePtr->gpio->speed = Speed_2MHz;
# 821 "src/main/drivers/pwm_mapping.c"
        if (type == MAP_TO_PWM_INPUT && !init->useParallelPWM)
            continue;

        if (type == MAP_TO_PPM_INPUT && !init->usePPM)
            continue;
# 928 "src/main/drivers/pwm_mapping.c"
...
# 981 "src/main/drivers/pwm_mapping.c"
        } else {
            continue;
        }
